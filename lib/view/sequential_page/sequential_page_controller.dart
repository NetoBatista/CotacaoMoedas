...
class SequentialPageController {
  ...
  TextEditingController countSequentialController = TextEditingController();
  ...
  void verifyCanBeSearch(){
    ...
    if(countSequentialController.text.length == 0){
      canBeSearch.value = false;
      return;
    }
  ...
  Future getSequentialOcurrence() async {
    getOcurrence.value = true;

    var startDate = DateFormat("yyyyMMdd", "ptBR").format(DateTime(startDateSelected?.year as int, startDateSelected?.month as int, startDateSelected?.day as int));
    var endDate = DateFormat("yyyyMMdd", "ptBR").format(DateTime(endDateSelected?.year as int, endDateSelected?.month as int, endDateSelected?.day as int));

    listAllOcurrence.clear();
    var response = await _ocurrenceService.sequencialDate((coinSelected?.name).toString(), int.parse(countSequentialController.text), startDate, endDate);
